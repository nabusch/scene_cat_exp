fn_generate_input_mem_separateblocks <- function(vars, dirs, isubject){
 ...
 available_images <- available_images %>% anti_join(., cat_trials, by = "stimulus") 
 ...
 # -------------------------------------------------------------------------
 all_catch_files <- list.files(sprintf("%s/", dirs$images), pattern = "\\.jpg$", full.names = TRUE)
 ...
 write_xlsx(mem_trials, filename, col_names =TRUE)
