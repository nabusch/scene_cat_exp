...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/teresaberther/Psychologie/AE_Busch/Niko/Memorability_Exp_V1/SceneCat_2023_1_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    start_memotest_text = visual.TextStim(win=win, name='start_memotest_text',
        ...
        languageStyle='LTR',
        depth=0.0);
    start_memotest_resp = keyboard.Keyboard()
    ...
    for thisCategorization_trial in categorization_trials:
        ...
        kitchen_a_categorization_trials = data.TrialHandler(nReps=kitchen_1, method='sequential', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions(conditions_kitchen_a),
        ...
        kitchen_b_categorization_trials = data.TrialHandler(nReps=kitchen_2, method='sequential', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions(conditions_kitchen_b),
        ...
        living_a_categorization_trials = data.TrialHandler(nReps=living_room_1, method='sequential', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions(conditions_living_a),
        ...
        living_b_categorization_trials = data.TrialHandler(nReps=living_room_2, method='sequential', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions(conditions_living_b),
        ...
        bed_a_categorization_trials = data.TrialHandler(nReps=bedroom_1, method='sequential', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions(conditions_sleep_a),
        ...
        bed_b_categorization_trials = data.TrialHandler(nReps=bedroom_2, method='random', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions(conditions_sleep_b),
    ...
    # keep track of which components have finished
    start_memotestComponents = [start_memotest_text, start_memotest_resp]
    ...
    while continueRoutine:
        ...
        # if start_memotest_text is starting this frame...
        if start_memotest_text.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
            # keep track of start time/frame for later
            start_memotest_text.frameNStart = frameN  # exact frame index
            start_memotest_text.tStart = t  # local t and not account for scr refresh
            start_memotest_text.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(start_memotest_text, 'tStartRefresh')  # time at next scr refresh
            # update status
            start_memotest_text.status = STARTED
            start_memotest_text.setAutoDraw(True)
        
        # if start_memotest_text is active this frame...
        if start_memotest_text.status == STARTED:
            # update params
            pass
        ...
        # if start_memotest_resp is starting this frame...
        if start_memotest_resp.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
            # keep track of start time/frame for later
            start_memotest_resp.frameNStart = frameN  # exact frame index
            start_memotest_resp.tStart = t  # local t and not account for scr refresh
            start_memotest_resp.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(start_memotest_resp, 'tStartRefresh')  # time at next scr refresh
            # update status
            start_memotest_resp.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(start_memotest_resp.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(start_memotest_resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if start_memotest_resp.status == STARTED and not waitOnFlip:
            theseKeys = start_memotest_resp.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            _start_memotest_resp_allKeys.extend(theseKeys)
            if len(_start_memotest_resp_allKeys):
                start_memotest_resp.keys = _start_memotest_resp_allKeys[-1].name  # just the last key pressed
                start_memotest_resp.rt = _start_memotest_resp_allKeys[-1].rt
                start_memotest_resp.duration = _start_memotest_resp_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    # set up handler to look after randomisation of conditions etc
    memotest_trials = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(conditions_memo_test),
        seed=None, name='memotest_trials')
    thisExp.addLoop(memotest_trials)  # add the loop to the experiment
    thisMemotest_trial = memotest_trials.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisMemotest_trial.rgb)
    if thisMemotest_trial != None:
        for paramName in thisMemotest_trial:
            globals()[paramName] = thisMemotest_trial[paramName]
    
    for thisMemotest_trial in memotest_trials:
        currentLoop = memotest_trials
        thisExp.timestampOnFlip(win, 'thisRow.t')
        # pause experiment here if requested
        if thisExp.status == PAUSED:
            pauseExperiment(
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 1.0 repeats of 'memotest_trials'
